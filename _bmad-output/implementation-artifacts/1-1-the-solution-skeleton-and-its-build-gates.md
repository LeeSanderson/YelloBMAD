...
### Review Findings — third pass (2026-08-27)

Code review of `9459d4b..65c3e50` — the commit that closed the second pass's 32 findings, reviewed
on its own terms. 20 files, +1,722/−353. Three parallel adversarial layers again, all on
`claude-opus-5[1m]`. **Every HIGH below was demonstrated by planting into the real tree or a
byte-copy of it, not reasoned.** The tree was restored and re-verified after each.

**Baseline independently re-verified, and every claim the commit makes is true.**
`dotnet build` clean at 0 warnings (including `--no-incremental`); `dotnet test Yello.slnx`
52 passed / 0 failed / 0 skipped / exit 0; architecture suite exactly 47, Slices exactly 5; all
six trait counts exact (AR-1 19, AR-2 25, AR-4 6, AR-35 4, NFR-1 2, P0 47); the three empty
suites each report `Total: 0` and exit 0; 18 of the 24 plants independently reproduced and all 18
caught by the named assertion; the negative check reproduced. The story's AC1–AC5 now match
`epics.md` verbatim, and the second AC5 amendment is properly recorded. **AC1, AC2, AC3 and AC5
are met**, AC3 notably stronger — the `SolutionAssemblies` fix is real and Gate C genuinely covers
`tests/**` now, which was the second pass's highest-severity finding and is properly closed.
Scope is clean on every line of the boundary table, and the `#blazor-error-ui` variance is
honestly recorded and does not pre-empt story 1.2.

#### The finding that matters more than the list: this is not converging by patching

Three passes have now found the same defect, one level along each time:

| Pass | Found | Fixed by | Next pass found |
|---|---|---|---|
| 1 | gates assert less than their names claim | 33 patches | the fixes had the same defect |
| 2 | the fixes assert less than they claim; **plants validated the cited example, not the sibling route** | 32 patches | the same, again |
| 3 | the fixes assert less than they claim; plants still validate the cited route; **and the fixes introduced a new defect class** | — | — |

The second pass's own headline was "planting the cited example proves the gate fires, not that
the ban is complete". This pass planted `$(...)` — the spelling the finding named — and left
`@(...)`, `%(...)`  and `%XX` escapes open. **Blind Hunter demonstrated EF Core landing inside
`Yello.Domain`, the ring that "must reference NOTHING", with the suite reporting 47/47 green.**
Writing more patches in the same style will produce a fourth pass with the same shape.

**The root cause is structural, and it is one sentence.** These gates read *declared* MSBuild XML
as text and reason about it as though it were *evaluated* build state. MSBuild offers many
indirections between the two — `$(property)`, `@(item)`, `%(metadata)`, `%XX` escapes,
`Condition` overrides, values assembled from other properties, `Directory.Build.targets`
overriding `Directory.Packages.props`, and `Sdk=` imports — and each is a separate bypass. Every
pass closes one spelling. Twelve of the findings below are that one cause. The mechanism that
collapses them is asking MSBuild instead of parsing XML: `dotnet msbuild -getItem:` /
`-getProperty:` returns evaluated values and is immune to indirections nobody has thought of yet.
That was noted as "the thorough fix" when the second pass applied the `$(`-only check, and
deferred. It should not be deferred a third time. **Raised as a decision, not a patch.**

**A second, new defect class appeared in this pass, and it is the mirror image.** Raw-*text*
scanning for "one source of truth" produces false positives, and one of them is unsatisfiable:

- Any `.cs` file containing the five-character literal `"yello"` — the product's own name — fails
  the build (demonstrated).
- Every literal-argument `GetConnectionString("...")` is reported as duplicating the Aspire
  resource name (demonstrated), which blocks story 1.10, a read replica, a cache, or a second
  Aspire resource. It is also now redundant, since the *value* is matched.
- `[Fact]` or `[Theory]` appearing **in prose** inside an empty suite creates a state no edit
  satisfies (demonstrated, both halves): the zero-test gate demands the switch be removed, and
  removing it makes `dotnet test` exit 8. The only way out is editing the gate. A collection
  definition or a convention note in the first file story 1.9 lands is exactly this shape.

An over-broad gate that fails a build nobody has broken is not a lesser defect than a permissive
one — it teaches developers to dismiss the suite, which is how a gate stops working.

#### Decisions needed

- [x] [Review][Decision] HIGH — **Evaluate MSBuild, or keep parsing its XML?** — **Resolved 2026-08-27 (Lee): hybrid — evaluate where indirection is demonstrated.** Use `dotnet msbuild -getItem:` / `-getProperty:` for the five questions where a bypass has been proven (package references, `Compile` items, the framework pin, the CPM switches, the test-platform arguments), and keep XML parsing where the value cannot be indirected. The reasoning behind the choice is worth keeping: full evaluation is the stronger guarantee, but this is the suite story 1.10 runs *first* in CI precisely because it finishes in about a second, and paying that cost on assertions with no demonstrated bypass buys nothing. Two consequences to honour when implementing: a gate failure must stay distinguishable from an MSBuild *invocation* failure (a gate that cannot answer is not a gate that passes — the `SolutionAssemblies` lesson), and the XML-parsing assertions that remain should say in their own comments why evaluation was not needed for them, so the split is legible rather than arbitrary. Original finding: Twelve findings below share one cause: the gates compare declared text where the build uses evaluated values. Demonstrated consequences include EF Core inside `Yello.Domain` with 47/47 green (`@(...)`), the zero-test switch applied through `$(PlantSwitch)` with the gate green, `CentralPackageTransitivePinningEnabled=false` from `Directory.Build.targets` with the gate green, a `Condition`-guarded override beating every "last unconditional" read, and `%2E%2E%2F`-escaped `Compile` items resolving outside their ring undetected. **Option A — evaluate:** replace the XML reads for item and property questions with `dotnet msbuild -getItem:` / `-getProperty:` per project. Closes the whole class including future spellings, and makes the gates test what the build does rather than what the file says. Costs: the architecture suite gains a dependency on invoking MSBuild (slower — currently ~1s for 47 assertions; this is the suite story 1.10 runs *first* in CI precisely because it is fast), and needs care so a gate failure is distinguishable from an MSBuild invocation failure. **Option B — keep parsing, widen the net:** add `@(`, `%(`, `%` escapes and conditional-override reporting as ordinary patches. Cheap and fast, and explicitly accepts a fourth pass finding the next indirection. **Option C — hybrid:** evaluate for the handful of questions where indirection is demonstrated (package references, `Compile` items, the framework pin, the CPM switches, the test-platform arguments) and keep XML parsing where the value cannot be indirected. This is the biggest open question in the suite the remaining 52 stories are gated by, and it is architecture rather than cleanup.
- [x] [Review][Decision] MEDIUM — **How aggressive should the "stated once" text scans be?** — **Resolved 2026-08-27 (Lee): narrow to the consuming projects and to string-literal positions.** Restrict each scan to the `.cs` files of the projects that actually consume the value; match string-literal positions rather than raw text; drop the now-redundant `GetConnectionString`/`AddDatabase` regex, since matching the value closes the routes it was added for; and require the test-attribute match to be followed by a method-ish token. This keeps every bypass the scans genuinely closed while removing all three demonstrated false positives, including the unsatisfiable one. Original finding: Three demonstrated false positives, one unsatisfiable (above). The scans exist for a real reason — they closed the interpolation and `const` routes the regex missed — so the question is scope, not whether. Options: (a) restrict to the `.cs` files of the projects that actually consume each value, and to string-literal positions rather than raw text; (b) keep the value match, drop the now-redundant `GetConnectionString`/`AddDatabase` regex, and require the test-attribute match to be followed by a method-ish token; (c) keep as-is and accept that later stories will hit them. Note the `[Fact]`-in-prose deadlock has to be fixed under any option, because no edit to the *project* resolves it.
- [x] [Review][Decision] MEDIUM — **AC4: what evidences that the Host actually runs the check, and that `aspire run` still wires the graph?** — **Resolved 2026-08-27 (Lee): a host-startup integration test.** Boot the real `WebApplication` against the shared fixture's container and assert the log, so the test exercises `Program.cs` rather than calling `RunAsync` directly. That closes all three failure modes the comment already claims — deletion, an inverted guard, and a dropped `await` — which is what ends three passes of reworking AC4's evidence. The `aspire run` clause stays evidenced by the manual transcript: closing it needs Aspire's testing package, which is outside the AR-1 table and therefore an architecture edit rather than a review patch. Record that boundary explicitly so the next pass does not re-raise it as an oversight. Original finding: Demonstrated: deleting the whole `await StartupConnectivityCheck.RunAsync(...)` block from `Yello.Host/Program.cs` leaves **52/52 green**. The new test calls `RunAsync` directly and never touches `Program.cs`, so of the three failure modes the comment at `Yello.Host/Program.cs:19-23` claims are now caught — "deleting the call, inverting the environment guard inside it or dropping this await" — only the middle one is. Separately, **no test in the solution touches `Yello.AppHost`**: `AddDatabase`, `WithReference`, `WaitFor` and `SplitImageReference` are asserted by nothing, and AC4's `aspire run` clause still rests on a transcript the record itself calls stale. This is the third pass in which AC4's evidence has been reworked, which is the argument for choosing a shape that ends it. Options: (a) a source-level gate asserting `Program.cs` invokes the check and awaits it — cheap, catches deletion, cannot catch semantics; (b) a host-startup integration test that boots the real `WebApplication` against the fixture's container and asserts the log; (c) an AppHost test using Aspire's testing package — closes the `aspire run` clause too, but adds a package outside the AR-1 table; (d) accept, and correct the overclaiming comment to say only what is true.

#### Patches

*Twelve of these are contingent on the first decision and are marked ⓘ — under Option A they are
subsumed rather than applied individually.*

- [x] [Review][Patch] HIGH — ⓘ **`@(item)` is the unclosed sibling of `$(property)`, and it breaches the innermost ring with the suite green** [tests/Yello.Tests.Architecture/RepositoryLayout.cs:263]. `UnresolvableIncludes` tests `include.Contains("$(")` only. **Demonstrated:** `<SharedSrc Include="..\Yello.Infrastructure\AssemblyMarker.cs" />` + `<Compile Include="@(SharedSrc)" />` and `<Orm Include="Microsoft.EntityFrameworkCore" />` + `<PackageReference Include="@(Orm)" />`, both planted in `Yello.Domain` — the ring that may reference nothing — build successfully, `-getItem:PackageReference` confirms EF Core is a real reference, and the suite reports **47/47, Failed: 0**. Swapping `@(` for `$(` fails two assertions immediately, so the gate is spelling-specific rather than mechanism-specific. Worse than a miss for `Compile`: `Path.Combine(projectDirectory, "@(SharedSrc)")` resolves *inside* the project, so the escape check affirmatively passes. Also open: `%(metadata)`, and `%XX` escapes (`%2E%2E%2F` — MSBuild resolves it to `../`, `Path.GetFullPath` on the literal reports inside the project; demonstrated).
- [x] [Review][Patch] HIGH — **A duplicate `<AssemblyMetadata>` serves the wrong SQL Server engine to every suite, gate green** [tests/Yello.Tests.Architecture/TestingConventionTests.cs:177, :183]. Both checks use `metadata.Exists(...)` — an *any* check — while both readers (`BuildConstants`, `SqlServerContainerFixture`) use `FirstOrDefault`. **Demonstrated:** a second item for the same key placed *before* the correct one makes `SqlServerContainerFixture.Image` return `…:2019-latest` while the suite reports 47/47 green — including the assertion whose own comment says "the metadata exists, the gate was satisfied, and the value stamped is empty". SQL Server 2019 has no `SESSION_CONTEXT`/RLS parity and no `Latin1_General_100_BIN2` verification, which is exactly what centralising the value was meant to make impossible. Plant 7 used the `Value`-emptied route; this is its sibling. Fix: assert *exactly one* `AssemblyMetadata` per key — first-wins is the reader's semantics — which is the same "exactly-one, not any" hardening this commit applied to the CPM switches.
- [x] [Review][Patch] HIGH — **The broadened fixture catch turns "the shared value is unreadable" into "no container runtime", so AC4's evidence skips instead of failing** [tests/Yello.Tests.Shared/SqlServerContainerFixture.cs:259]. `IsContainerRuntimeAvailable` now catches `ArgumentException or InvalidOperationException`, and `BuildContainer()` → `Image` → `ReadMetadata` sits inside that `try` — where a missing key throws `InvalidOperationException`. **Demonstrated** with a runtime confirmed reachable: changing the fixture's key literal flips the method from `True` to `False`, so all three container-backed cases skip with "No container runtime is reachable", reporting a build-stamp defect as a local Rancher problem and silently removing the container-backed half of AC4's new evidence. This is a regression introduced by the second pass's own fix, and the method's doc claims an "honest split" of two cases while there are three. Fix: read `Image` before the `try`, or catch only Testcontainers-shaped failures and let a metadata failure escape.
- [x] [Review][Patch] HIGH — ⓘ **A `Condition`-guarded override beats every "last unconditional" read in the suite** [tests/Yello.Tests.Architecture/PackageVersionPinTests.cs:170-182; TestingConventionTests.cs:149-213]. **Demonstrated:** an unconditional `true` followed by a `Condition`-guarded `false` whose condition fires evaluates to `false`, while the gate — which filters to unconditional declarations — reads `true` and passes. Same shape for `YelloSqlServerImage`, `YelloDatabaseResourceName` and the `AssemblyMetadata` items, so AC4's named engine can be asserted while the build discards it. The correct rule already exists in this commit as `ProjectFileGateTests.ConditionalFrameworkDeclarations` — report conditional declarations as their own problem, since no gate here evaluates conditions — and **was not carried across**, which is verbatim the criticism this pass's own patch notes levelled at the previous pass. Also correct the docstring: "MSBuild takes the last unconditional value" is not the rule.
- [x] [Review][Patch] HIGH — ⓘ **The CPM switch gate reads one file, and the repo's own guidance points elsewhere** [tests/Yello.Tests.Architecture/PackageVersionPinTests.cs:170]. It reads `Directory.Packages.props` only, while `Directory.Build.props:78-79` instructs future stories that "an override that needs to read a package property belongs in `Directory.Build.targets`, which is imported after". **Demonstrated:** a root `Directory.Build.targets` setting `CentralPackageTransitivePinningEnabled=false` yields `-getProperty` → `false`, `project.assets.json` → `SSH.NET/2024.2.0`, suite 47/47 green. Two corrections to the assertion's own docstring, both measured: it does *not* "state the invariant directly" (it states a fact about one file's text), and the claim that turning transitive pinning off leaves GHSA-q939-rpr3-3284 reinstated "with no NuGet backstop at all" is **wrong** — `dotnet build Yello.slnx` then fails with `error NU1903` ×4. Delete that clause.
- [x] [Review][Patch] HIGH — ⓘ **`--ignore-exit-code 8` behind an MSBuild property defeats the zero-test gate on a populated release-gating suite** [tests/Yello.Tests.Architecture/TestingConventionTests.cs:364-383]. `IgnoredExitCodes` tokenises the raw XML text and never expands properties — the identical defect `No_gated_item_hides_behind_an_MSBuild_property` was added in this same commit to close, scoped to item `Include` attributes only. **Demonstrated** on the populated `Yello.Tests.Slices`: `<PlantSwitch>--ignore-exit-code 8</PlantSwitch>` plus `$(PlantSwitch)` in the arguments makes MSBuild apply the switch while the gate reports 47/47 green. The four suites already use `$(TestingPlatformCommandLineArguments)` expansion idiomatically, so the shape is native to the file. Also open, and separately demonstrated: the `--ignore-exit-code=8` **equals** form ("Zero tests ran … passed", exit 0) which the space-delimited tokeniser never sees.
- [x] [Review][Patch] HIGH — **`[Fact]` in prose inside an empty suite creates a state no edit satisfies** [tests/Yello.Tests.Architecture/TestingConventionTests.cs:399]. The regex matches raw file text, comments and XML docs included. **Demonstrated, both halves:** a non-test helper in `Yello.Tests.Isolation` whose doc mentions "the `[Fact]` and `[Theory]` methods story 1.9 will add" makes the gate demand the switch be removed; removing it makes `dotnet test` return exit code 8. Both gates cannot be satisfied, and the only resolution is editing the gate — in the first file story 1.9 touches. Fix: require the match to be followed by a method-ish token, or exclude comment lines (the risk the docstring dismisses is the *string-literal* case, which is a different case). Under-match, separately demonstrated: `[Trait("a","b"), Fact]` — `Fact` not first in the list — and `[global::Xunit.Fact]` both read as absent.
- [x] [Review][Patch] MEDIUM — **Both "unplantable" claims are false, and both gates are plantable** [story record, *Task 9 (third pass)*]. This pass's own honest-looking exclusions do not hold, and the story's definition of done makes an unvalidated absence assertion not a test. (a) **`UserManager<>`'s role surface** — recorded as needing ASP.NET Core Identity, "which no project does until story 1.3". `Microsoft.Extensions.Identity.Core.dll`, which holds `UserManager<TUser>`, ships **in the `Microsoft.AspNetCore.App` shared framework** that `Yello.Host`'s web SDK already references. **Demonstrated:** an 11-line file calling `manager.IsInRoleAsync(user, "Admin")` with **no package change at all** builds clean and fails `No_code_references_Identitys_role_store_or_role_manager`, naming the site. (b) **`AppHost_types_depend_on_no_Yello_assembly`** — recorded as unplantable because Aspire marks project resources `ReferenceOutputAssembly=false`. True of the two existing edges, but the rule is about *any* Yello assembly: a **third** reference with `IsAspireProjectResource="false"` plus one type use builds clean and fails both that rule and the edge table. `RingDependencyTests.cs:238-241` already says "a type dependency appearing here means someone has turned that off" — the file documents the route the record calls nonexistent. Fix: plant both, and delete the exclusions.
- [x] [Review][Patch] MEDIUM — **`ClassifyRoleClaim`'s `FieldReference` branch is dead code and its stated mechanism is wrong** [tests/Yello.Tests.Architecture/RoleApiScan.cs:369-383]. `System.Security.Claims.ClaimTypes.Role` is a **`const`** (verified: `IsLiteral == True`), so the compiler emits `ldstr` and never `ldsfld`. The branch keying on a `FieldReference` can therefore never fire, and the remarks asserting "`ClaimTypes.Role` is a field load" and "`ClassifyCall` exits at its `is not MethodReference` guard" are factually wrong. The ban is functionally intact — the URI-literal branch catches it — but one branch of a new gate is unreachable and unvalidated, plant-table rows 17 and 18 validate the **same** route so "24 violations, each caught" over-counts distinct routes by one, and the failure message tells the likeliest real offender they wrote "a literal URI" when they wrote `ClaimTypes.Role`. Fix: delete the dead branch, match the URI *suffix* rather than the exact literal (closing the two-piece assembly this file's own `RoleClaimTypeUri` demonstrates), and correct the remarks and the plant table.
- [x] [Review][Patch] MEDIUM — **Gate C: two more first-party role routes** [tests/Yello.Tests.Architecture/RoleApiScan.cs:431-437, :422-430]. (a) `set_Roles` is resolved through `IsAuthorizeAttribute`, which walks this solution's base-type map, so `IAuthorizeData data = new AuthorizeAttribute(); data.Roles = "Admin";` emits `callvirt` on `Microsoft.AspNetCore.Authorization.IAuthorizeData` — neither `AuthorizeAttribute` nor in the map, and not named `*Role*` so the type scan misses it too. `RequireAuthorization(params IAuthorizeData[])` is the Minimal-API overload and `Yello.Host` is a Minimal-API host. (b) A subclass of `UserManager<T>` overriding `IsInRoleAsync` escapes the prefix match on the declaring type. Both are one accepted-declaring-type addition each. Note the genuinely out-of-reach routes (a bespoke `"role"` claim, `ClaimsIdentityOptions.RoleClaimType`) *are* honestly declared as known limits; these two were not.
- [x] [Review][Patch] MEDIUM — ⓘ **`Sdk=` attributes and `<Sdk>` elements are an import route no gate reads** [tests/Yello.Tests.Architecture/RepositoryLayout.cs:294-300; ProjectFileGateTests.cs:461-482]. The new `<Import>` ban closes explicit imports; an SDK import can carry a `GlobalPackageReference` or a framework property from a file nothing reads. `Yello.AppHost.csproj` already uses that route (`Aspire.AppHost.Sdk`), so it is established idiom here rather than hypothetical.
- [x] [Review][Patch] MEDIUM — **The Blazor WASM pins are documented as tracking `RuntimeFrameworkVersion` and nothing asserts it** [tests/Yello.Tests.Architecture/PackageVersionPinTests.cs:71-72; ProjectFileGateTests.cs:184]. Both hard-code `10.0.11` independently, and `Directory.Packages.props` says the WASM packages "must track the runtime patch pinned in `Directory.Build.props`". Bumping the runtime patch leaves the WASM pins behind with everything green — the same unasserted-coupling class this commit closed for the Aspire SDK attribute and the tool manifest. One line: derive the expected WASM version from the asserted `RuntimeFrameworkVersion`.
- [x] [Review][Patch] MEDIUM — ⓘ **The shared-value gate trims; MSBuild does not** [Directory.Build.props:59; tests/Yello.Tests.Architecture/TestingConventionTests.cs:200-213]. **Demonstrated:** a value written across lines evaluates to `"\n      mcr…\n    "` while the gate compares a trimmed copy and passes, so the stamped value is unusable and every container test then skips (compounding the fixture finding above). Assert the raw value carries no leading or trailing whitespace.
- [x] [Review][Patch] MEDIUM — **The inverted-guard case — the one thing said to need a container — needs no container** [tests/Yello.Tests.Slices/StartupConnectivityCheckTests.cs:103-106]. `RunAsync` returns at the environment check *before* reading configuration, so proving the guard held needs only `StubEnvironment("Production")` and any non-empty connection string: if the guard were inverted the check would log a connectivity failure rather than `ConnectivityCheckSkipped`, whether or not anything is listening. As written, the regression AC4's new coverage exists to catch is invisible on any machine or CI leg without a container runtime — in a suite the story states is not release-gating. Drop the `SkipUnless` from this one case so it runs everywhere.
- [x] [Review][Patch] MEDIUM — **Fixture: a failed start leaves a container running and blocks the retry that would explain it** [tests/Yello.Tests.Shared/SqlServerContainerFixture.cs:158-209, :163-168]. When `StartAsync` times out or throws, `_container` stays set and undisposed — a stray ~2 GB SQL Server for the rest of the run — and the new double-init guard then answers a retry with "InitializeAsync has already run" instead of the cause. The new tests mask it with `await using`; an `IClassFixture` consumer will not. Also: the guard is a non-atomic read-then-write, so two concurrent callers both pass it and one container is abandoned. Fix: dispose and clear on a failed start, and use `Interlocked.Exchange`.
- [x] [Review][Patch] MEDIUM — **`StartupConnectivityCheck` states a "never throws" contract it does not hold** [Yello.Host/StartupConnectivityCheck.cs:84-92, :156-170; Yello.Host/Program.cs:24]. The final `catch` allow-lists four exception types; anything else — including the exceptions Entra/AAD authentication modes raise — escapes, and `Program.cs` has no handler, so the Host dies before Kestrel binds. That is the outcome the class's own summary says is designed out. Fix: catch broadly in the outermost handler and log, since the contract is the reason the class exists.
- [x] [Review][Patch] MEDIUM — **Record accuracy, four items the previous pass marked closed.** (a) **File List still incomplete:** `docs/bmad-coverage.md` was modified in `155f4ce` — this story's own work — and appears in neither the File List nor the Change Log; the note added last pass says "a `git diff --stat` against the baseline is the check worth running", and running it finds a third omission. Its preamble also still says "every path below is new except those marked *modified*" when there are now four. (b) **The Change Log's own 2026-08-27 row contradicts itself**: "30 patches" in one clause and "all 32 patches applied" in the next; 32 is correct. (c) **Stale arithmetic three lines below the corrected figure**: "Of the 44, 10 are the A-series … the other 34" should read 47 and 37 — directly beneath a comment this commit added saying "do not restate the assertion count in prose anywhere else. It has been wrong in this comment twice." (d) **The drift cluster is two-thirds applied**: `Yello.Tests.Architecture.csproj` still omits `TestingConventionTests` from its Gate A list, while the File List describes that file as "Gate A / AR-35".
- [x] [Review][Patch] LOW — **The "every plant is the sibling route" claim is false** [story record, *Task 9 (third pass)*, preamble]. Plant 9 used `$(...)`, the spelling the finding cited, leaving `@(...)` open; plant 7 used the `Value`-emptied route, the cited example, leaving the duplicate-item route open; plants 2, 10, 11, 17 and 18 are also the cited route. This is the third consecutive occurrence of the blanket-coverage-claim defect — which this same section raised against the previous pass. Narrow it to what the table supports, in the shape the two honest exclusions already use.
- [x] [Review][Patch] LOW — **Two justifications in the record do not hold.** (a) The `IsContainerRuntimeAvailable` disposition says a daemon ping is out of reach because the client "would add an **unpinned** package, which Gate A correctly forbids" — `Directory.Packages.props` exists precisely to pin transitively-reached packages, and this repo already forward-pins `SSH.NET` 2026.0.0 that way. Gate A forbids an unpinned package, not a direct one. The consequence still stands, so the disposition may not change; the reason must. (b) The digest instruction at `Yello.AppHost/Program.cs:57-58` names two of the **three** files a digest pin must change — `TestingConventionTests.ExpectedSqlServerImage` and `Directory.Build.props:59` are the others — so a story 1.5/2.6 developer following it will hit a gate failure the message implied was handled.
- [x] [Review][Patch] LOW — **Robustness and accuracy cluster.** `LoadJson` rejects JSON the SDK accepts (a `//` comment or trailing comma in `global.json` resolves fine but fails the gate — use `JsonCommentHandling.Skip` and `AllowTrailingCommas`); `TryGetProperty` on a non-object `tools`/`aspire.cli` throws naming neither file nor remedy, which is what `LoadJson` exists to prevent; `SplitImageReference` accepts empty image or tag components (`registry/img:`, `registry/:tag`) that Testcontainers rejects, so the two diverge again; `ConnectivityAbandoned` (EventId 1006) is unreachable in production because `ApplicationStopping` cannot fire before `app.RunAsync()`, and no test covers it — add a test with a pre-cancelled token or mark it defensive; the deadline-versus-shutdown branch samples the token after the await so a real timeout can be reported as a clean shutdown, and the 15s deadline races an equal `ConnectTimeout` so an unreachable engine logs 1004 or 1002 nondeterministically; an unconditional `<TargetFrameworks>` in the root props file is read by nothing (verified inert today — it does not multi-target — so this is a gap rather than a hole); the `IsTestProject` `OutputType` fallback does not cover the scenario its own comment cites, since xunit.v3 4.0.0 ships no props or targets and nothing sets the property implicitly today; `Yello.AppHost/Program.cs:41` still points readers at `Yello.Host/Program.cs` for logic this commit moved to `StartupConnectivityCheck.cs`; the fixture's precondition message tells consumers to use `IClassFixture`/`ICollectionFixture` while all three consumers construct it directly, which also means `Yello.Tests.Slices` now starts three separate SQL Server containers; and AC4's new tests carry `Priority=P1`, placing them outside the P0 tier story 1.10 runs first, while tracing to `Requirement=AR-1` though their own docs argue AR-33 and AR-36.
- [x] [Review][Patch] LOW — **Uncommitted verification debris was left in the repository by the second pass** [repo root; `tests/Yello.Tests.Architecture/`]. Two `DevPersonalYelloBMADartifacts/` directories, 70 files and 1.9 MB, residue of a `-p:ArtifactsPath=` probe whose Windows path was mangled by shell escaping. Invisible to `git status` only because `.gitignore`'s `obj/` swallowed their only contents, and `RepositoryLayout.ExcludedDirectories` lists `artifacts`, not that name — so had they ever held a `.csproj` they would have failed the inventory gate. **Already removed 2026-08-27**; recorded because a pass that claims a clean tree should not leave any, and because the exclusion list matching on an exact name is a latent sharp edge.

#### Dismissed (2)

1. **`StartupConnectivityCheck.RunAsync` changing AC4's behaviour when it moved out of `Program.cs`** — checked line by line by the Acceptance Auditor and confirmed unchanged: same position before `app.RunAsync()`, still awaited, still Development-only, still one connection with pooling off and a 15s deadline, still no probe, no timer and no migration. AR-33 and AR-36 hold. The two behavioural changes are both intended fixes (the metadata read moved below the environment guard; the skip log moved `Debug`→`Information`, which the shipped configuration does emit).
2. **`RoleApiScan`'s self-scan avoidance being fragile** — the `string.Concat` assembly of `RoleClaimTypeUri`, `SqlServerImageLiteral` and `ExpectedSqlServerImage` works, and 47/47 green proves it. The *evasion* this implies for a third party is a real finding and is recorded under the `ClaimTypes.Role` patch above; the self-avoidance itself is sound.
...
| `dotnet test Yello.slnx` | `Passed!  total: 54  failed: 0  skipped: 0` — exit 0 |

**Assertion counts, after the third review pass (2026-08-27).** 47 in
`Yello.Tests.Architecture` and 7 in `Yello.Tests.Slices`; the three suites that are still
genuinely empty report zero tests and still exit 0. The progression is 26 → 44 → 47 → 47 for the
architecture suite, and 0 → 2 → 5 → 7 for Slices. The architecture count did not move in the
third pass because that pass hardened nineteen existing assertions rather than adding new ones -
five of them now reading evaluated MSBuild state instead of declared text, which is a change of
mechanism rather than of count. The three new architecture assertions are
...
Of the 47, **10 are the A-series** this story owes — A-1 (ring rule, 4), A-2 (EF/ASP.NET leak,
2), A-3 (Role-API ban, 4). The other 37 sit outside the A-1…A-15 numbering, because that series
...
#### Task 9 (fourth pass) — planted-violation results for the third review pass

Twenty violations planted 2026-08-27, **all twenty caught by a named assertion**, plus two
negative checks confirming that the two over-broad gates no longer fail on legitimate content.
The tree was restored and re-verified after each. Three plants initially returned a build failure
for an incidental analyser reason and were re-planted properly rather than counted — a build that
fails for the wrong reason is not validation.

**What this pass's plants do differently.** Each closes a mechanism rather than a spelling,
because that was the third pass's finding: five of the assertions below now read what MSBuild
*evaluates* instead of what a file *declares*, so the plant is the mechanism (indirection,
conditional override, a value set from another file) rather than one syntax for it.

| # | Planted violation | Caught by |
|---|---|---|
| 1 | `<PackageReference Include="@(Orm)" />` resolving to EF Core inside `Yello.Domain` | `No_project_references_a_package_its_ring_forbids` (evaluated) + the indirection check |
| 2 | `<Compile Include="@(SharedSrc)" />` reaching outside the project | `No_project_compiles_source_from_outside_its_own_directory` (evaluated `FullPath`) + the indirection check |
| 3 | `<Compile Include="%2E%2E%2FPlantEscaped%2F…" />` — MSBuild's escape for `../` | the same two; MSBuild resolved it to `../PlantEscaped/…` and the gate saw the resolved path |
| 4 | `CentralPackageTransitivePinningEnabled=false` inside a `Condition` that fires | `Central_package_management_and_transitive_pinning_are_enabled` (evaluated per project) |
| 5 | the same switch set from a root `Directory.Build.targets` — the file this repo's own guidance points at | the same |
| 6 | `RuntimeFrameworkVersion` overridden inside a firing `Condition` in the root props file | `The_target_framework_and_runtime_are_pinned_in_exactly_one_unconditional_place` (evaluated per project) |
| 7 | a **duplicate** `AssemblyMetadata` for the image key, placed first, naming a 2019 engine | `Values_shared_between_projects_are_stated_once_in_the_build` (exactly-one, not any) |
| 8 | the shared image value written across lines, so MSBuild stamps whitespace | the same |
| 9 | a `Condition`-guarded shared-value declaration alongside the unconditional one | the same |
| 10 | `--ignore-exit-code 8` behind `$(PlantSwitch)` on the **populated** Slices suite | `Only_suites_with_no_tests_may_ignore_the_zero_test_exit_code` (evaluated arguments) |
| 11 | the same switch in the `--ignore-exit-code=8` form | the same (the flag's value is parsed, not substring-matched) |
| 12 | `[Trait("Suite","Merge"), Fact]` — the attribute **not first** in its list | the same |
| 13 | `principal.HasClaim(ClaimTypes.Role, "Admin")` in `Yello.Host` | `No_code_applies_Authorize_with_a_Roles_argument` |
| 14 | `manager.IsInRoleAsync(user, "Admin")` — **recorded last pass as unplantable**, with no package added | `No_code_references_Identitys_role_store_or_role_manager` |
| 15 | a third AppHost `ProjectReference` with `IsAspireProjectResource="false"` plus a type use — **recorded last pass as unplantable by construction** | `AppHost_types_depend_on_no_Yello_assembly` + the edge table |
| 16 | `<Sdk Name="Some.Vendored.Sdk" />` — an import route no gate read | `No_project_declares_a_raw_assembly_reference_or_an_explicit_import` |
| 17 | a Blazor WASM pin moved off the runtime patch it is documented to track | `Directory_Packages_props_pins_exactly_the_expected_set…` |
| 18 | `"tools": []` in the tool manifest — a shape that used to throw rather than report | `Every_ungoverned_copy_of_the_Aspire_version_matches_the_pin` |
| 19 | the whole `await StartupConnectivityCheck.RunAsync(...)` block deleted from `Program.cs` | both `HostStartupTests` cases — **this left 52/52 green before this pass** |
| 20 | `IsDevelopment()` inverted inside the check | five cases across `HostStartupTests` and `StartupConnectivityCheckTests` |
| — | **negative:** `[Fact]` mentioned in a doc comment inside an empty suite | **nothing** — 47/47 green. Before this pass this created a state no edit to the project could satisfy. |
| — | **negative:** `global.json` carrying a `//` comment, which the SDK honours | **nothing** — 47/47 green, and `dotnet --version` still resolves 10.0.303. |

**Plant 16 also demonstrated the evaluation helper failing loudly**, which matters as much as the
gate it serves: an unresolvable SDK makes `dotnet msbuild` exit non-zero, `MsBuildEvaluation`
throws naming the project, and every evaluated assertion fails rather than quietly reporting an
empty item set. That is the `SolutionAssemblies` lesson applied to new infrastructure — a gate
that cannot answer must not pass.

**One plant was investigated and found not to be a bypass at all.** `%(metadata)` in an `Include`
is not expanded outside a target: MSBuild passes the literal through and the compiler fails with
`CS2001`. So the `%(` arm of the indirection check is belt-and-braces rather than the closing of a
live hole, and is recorded that way instead of being counted as a caught plant. `$(...)`,
`@(...)` and `%XX` escapes are all real and all closed.
...
*Claim narrowed 2026-08-27 by the third pass.* This paragraph originally ended "Every plant below
is therefore the sibling route, not the cited one", and that was not true of all of them —
plants 2, 7, 9, 10, 11, 17 and 18 are the cited route. Plant 9 used `$(...)`, the spelling the
finding named, and `@(...)` was then demonstrated to put EF Core inside `Yello.Domain` with all 47
assertions green; plant 7 emptied an `AssemblyMetadata` `Value`, the cited example, leaving the
duplicate-item route open. **Third consecutive occurrence of the blanket-coverage-claim defect**,
which this same section raised against the pass before it. What the table below supports is that
each listed plant was caught by the assertion named beside it — nothing broader.
...
~~**Two assertions remain unplanted, stated rather than implied**~~ — **both claims were false,
and both gates were planted by the third pass (2026-08-27).** Kept here rather than deleted,
because being wrong about *why* something cannot be validated is a more interesting failure than
not validating it: both exclusions read as careful and were simply insufficiently curious.

1. ~~**`UserManager<>`'s role surface.** Planting it requires referencing ASP.NET Core Identity,
   which no project does until **story 1.3** wires it.~~ **False.**
   `Microsoft.Extensions.Identity.Core`, which holds `UserManager<TUser>`, ships **inside the
   `Microsoft.AspNetCore.App` shared framework** that `Yello.Host`'s web SDK already references.
   Planted with an eleven-line file and **no package change of any kind**: the build stayed clean
   and `No_code_references_Identitys_role_store_or_role_manager` failed, naming the call site.
2. ~~**`AppHost_types_depend_on_no_Yello_assembly`.** Unplantable by construction: the AppHost's
   project references are Aspire project *resources* marked `ReferenceOutputAssembly=false`, so
   it cannot compile against a Yello type at all.~~ **False for the rule as written.** That is
   true of the two *existing* edges, and flipping either breaks the AppHost's own compile — but
   the rule is about *any* Yello assembly, and a **third** reference carrying
   `IsAspireProjectResource="false"` compiles fine. Planted: the build stayed clean and both
   `AppHost_types_depend_on_no_Yello_assembly` and the edge table failed. `RingDependencyTests`
   already says "a type dependency appearing here means someone has turned that off" — the file
   documented the route the record called nonexistent.
...
- `docs/bmad-coverage.md` — *modified* in `155f4ce`, recording the `bmad-testarch-framework` halt
  and its resolution. *Previously unlisted; added 2026-08-27.* Found by the very check the note
  below recommends, which is the argument for running it rather than recommending it.
...
| 2026-08-27 | **Second code review of `fd556ff..HEAD`** — the pass that closed the first 33 findings, reviewed on its own terms by three parallel adversarial layers. Baseline independently re-verified before triage, and every numeric claim in this record checked out (44 assertions, all six trait counts, the 26→44 delta); the shared fixture was confirmed to genuinely run by reading the Docker event log rather than the record, which also confirmed `sqlcmd` is back in the wait chain. 3 decisions resolved by Lee, 32 patches, 5 dismissed, 0 deferred. **The dominant finding recurses the first pass's:** several of the gates written to fix "asserts less than it claims" had the same defect, and in two cases the new file's own docstring stated the property its code failed to hold — `SolutionAssemblies.cs` was added to stop Gate C narrowing its scope in silence and `continue`d, so under one output layout the suite reported 44/44 green having scanned **zero** test assemblies. A second, mechanical finding: the previous pass's plants validated the *example each finding cited* rather than the *sibling route the fix left open* — `GlobalPackageReference` in `Directory.Build.props`, `[Theory, InlineData]`, and a `Condition`-guarded framework pin were all still reachable. Also closed: Gate C's two remaining first-party role routes (the role claim, and `UserManager<>`'s role surface), MSBuild property indirection defeating every literal-matching ban, a version-less or `Condition`-guarded `PackageVersion` vanishing from the pin set, AC4's named image being asserted by nothing after centralisation, and a `TypeInitializationException` hazard the test suite documents at length and the Host had reintroduced. AC4 gained real automated coverage in `Yello.Tests.Slices` — including the inverted-guard case a source-level gate cannot express — which supersedes the manual `aspire run` transcript that had gone stale behind two rewrites. 24 violations planted and reverted, all 24 caught by a named assertion, plus one negative check; two assertions are recorded as unplantable with reasons rather than claimed. `dotnet build` clean at 0 warnings, `dotnet test Yello.slnx` 52 passed / 0 skipped / exit 0, architecture suite 44 → 47. Status moved `review` → **`done`**: all 3 decisions resolved by Lee, all 32 patches applied, nothing deferred, no unresolved finding at any severity. |
| 2026-08-27 | **Third code review of `9459d4b..65c3e50`** — the pass that closed the second review's 32 findings, reviewed on its own terms. Everything the commit claimed was verified true (52/52, 47 assertions, all six trait counts, 18 of 24 plants independently reproduced), and AC1, AC2, AC3 and AC5 confirmed met. But the recursion held a third time and the subject was the previous pass's own work: two HIGHs indicted comments written in that commit — deleting AC4's whole check from `Program.cs` left **52/52 green** while the comment claimed the new tests caught exactly that, and the central-package-management fix filtered conditional declarations *out* instead of reporting them, so a firing `Condition` still beat it. **Both "unplantable, recorded with reasons" claims were false**: `UserManager<>` needs no package (Identity ships in the shared framework the Host already references) and the AppHost rule is plantable through a third reference marked `IsAspireProjectResource="false"`; both were planted and both gates fired. A *new* defect class also appeared — over-broad raw-text scanning, one instance of which was **unsatisfiable**: `[Fact]` mentioned in a doc comment inside an empty suite made one gate demand the zero-test switch be removed while removing it made `dotnet test` exit 8. **The structural finding outranked the list:** twelve demonstrated bypasses shared one cause — the gates compared *declared* MSBuild text where the build uses *evaluated* values, so `$(...)`, `@(...)`, `%XX` escapes, `Condition` overrides, `Directory.Build.targets` and `Sdk=` imports were each a separate hole and each pass closed one spelling. Demonstrated at its worst: EF Core inside `Yello.Domain`, the ring that may reference nothing, with all 47 assertions green. Lee chose the hybrid remedy, and `MsBuildEvaluation` now answers the five questions where a bypass was proven — package references, `Compile` items, the framework pin, the CPM switches and the test-platform arguments — while declared-XML reads keep the questions that are genuinely about what a file states. Costs about two seconds for the whole solution, run in parallel. AC4 gained a host-startup integration test that boots the real `WebApplication`, closing the deletion and dropped-`await` cases a direct method call never could. 3 decisions resolved by Lee, 21 patches, 2 dismissed, 0 deferred; 20 violations planted and all 20 caught, plus two negative checks proving the over-broad gates no longer fail on legitimate content. `dotnet build` clean at 0 warnings, `dotnet test Yello.slnx` 54 passed / 0 skipped / exit 0. |
