...
**And** `px` never sizes or spaces type in any file, and outside the token layer every length comes from a token — the token layer itself states the spacing scale, the target floor, hairlines, radii, outline offsets and the long-press slop in `px` *(this clause was amended in `epics.md:542` at code review 2026-08-27 under Lee's decision 2; it previously read "`px` appears only on hairlines, radii and outline offsets — never on type". Reproduced in its amended form from the second review, 2026-08-28, so this section matches the source it cites.)*
...
**Then** they are asserted as **bounded low** surface-adjacency ratios (~1.09 and ~1.10) — held strictly between 1.0 and 1.5 — and are **not** gated against any *contrast* threshold *(UX-DR7 was amended at code review 2026-08-27 under Lee's decision 3 to sanction the bound; reproduced in its amended form at the second review, 2026-08-28)*
...
- [x] [Review][Patch] DECISION 4 — **Implement device-pixel snapping for the hairline, and gate it** [Yello.Client/wwwroot/css/tokens.css:294]. `resolution` media queries rebinding `--border-hairline-width` to `ceil(1.5 × dppx) ÷ dppx` — 2px at 1dppx, 1.6px at 1.25, 2px at 1.5, ~1.714px at 1.75, 1.5px at 2, ~1.667px at 3 — so a structural edge always lands on an integer number of device pixels. Gate that the snapping block exists and that every rebinding stays ≥ `HairlineFloorPx`. **This patch was REVERTED at the second code review on 2026-08-28** (decision 2 there): AC7's snapping half is now a recorded deferral with UX as owner. Two claims in this bullet were also wrong and are corrected rather than deleted. "Borders get visibly heavier at four of six common scales" was **five of six** - against the canonical 1.5px only 2dppx is unchanged; 1, 1.25, 1.5, 1.75 and 3dppx are all heavier. And "the only patch that changes rendered output" was false: the same commit reverted the error-banner border to `--border-hairline`, widened the target-floor rule to twelve further selectors and both axes, and changed the `.dismiss` control. Note for the implementer: `CssCorpus.ReadTokenValues` is a flat last-wins map, so `TokenValues["border-hairline-width"]` will become whichever branch is written last — every branch must clear the floor, because the gate will only measure one of them.
...
### Review Findings — second pass (2026-08-28)

Reviewed `a87045b..e29d61c` — the commit that applied the first pass's 27 patches. 12 files, +1921/−104, 2784 diff lines. 1724 of the added lines are `CssCorpus.cs` and `DesignFoundationGateTests.cs`, so this pass is overwhelmingly a review of new gate logic written by the model that had just reviewed the story. Three adversarial layers ran in parallel; all three planted violations empirically rather than reading alone, and each restored the tree (verified clean at `e29d61c`).

**The first pass did not close the defect class it was written to close.** Three layers independently, with different plants, showed the same three things: the flagship new gate (`The_hairline_is_snapped_to_a_device_pixel_at_every_declared_scale`) detects only the *total absence* of snapping bands, so deleting the 1.25 / 1.5 / 1.75dppx bands — the exact three scales AC7 names — leaves the suite 75/75 green; `ExcludesLocale` is a positional substring test that both accepts a rule scoped *to* Turkish and lets one correctly-excluded selector branch launder every other branch in the same rule, which is verbatim the shape the first pass reported and marked fixed; and the AC8 target floor is satisfied by any one declaration anywhere, so the entire interactive selector list can be deleted with the gate green — and it is now *weaker* than before, because `AllDeclarations()` includes markup, so a single inline `style="min-height: var(--target-min)"` satisfies it.

**A second theme is new to this pass: four gates now fail correct code.** `Every_stylesheet_is_linked_by_the_host_page` demands an individual `<link>` for every `*.css` in the tree, which makes Blazor CSS isolation unbuildable — scoped `*.razor.css` *must not* be linked individually, the SDK bundles it — so epic 2's first component with scoped styles cannot satisfy it by any means. `transparent` was added to the colour-literal ban with no permitted alternative and none possible. The `var(--x, var(--fallback))` idiom is reported as an unresolved reference. And a custom property declared in markup fails the resolution gate, because the commit taught the corpus to *read* markup without teaching it to *declare* from markup.

**A third theme: the records now assert things that are false.** `deferred-work.md:14` was amended by the first pass to state that the line-height floor "is now gated" — it is bypassed by `120%`, `1.2em` and `normal`. The Change Log claims the five new gates were "all re-proved against planted violations"; the Task 5 evidence table has no row for any of them. Two Completion Notes state `epics.md` is unamended, in a commit that amended it three times. The amendment to UX-DR2 and AC3 asserts the token layer states outline offsets in px; no offset token exists anywhere, and `base.css:189` writes `outline-offset: 2px` as a bare literal that the confinement gate is configured not to notice — so the amendment removed the exemption from the requirement and left it in the gate.

#### Decisions needed

- [x] [Review][Decision] **Patch forward on top of `e29d61c`, or revert it and redo the first pass properly?** — **Resolved 2026-08-28 (Lee): patch forward.** The defective intermediate state stays in history and the fixes land as a follow-up commit. Consequence accepted: several of this pass's patches are patches *to* first-pass patches, so the two commits must be read together to understand any one gate. Original finding: The commit is on `main`. Of its 27 patches: the flagship new gate is vacuous, three widened gates remain bypassable by the same shapes they were written for, four gates block correct code, one product patch (`role="button"`) makes an ARIA promise the framework does not keep, and the story file plus the deferred-work ledger now carry false claims of verification. Patching forward means ~49 further patches layered on a commit whose test code is not trustworthy in the large; reverting means the story returns to its pre-review state with the first pass's findings intact and re-applied under this pass's evidence. Options: **(a)** patch forward; **(b)** revert `e29d61c` and re-apply both passes' findings together; **(c)** revert only the two test files and keep the product, documentation and ledger changes.
- [x] [Review][Decision] **The snapping collapses three distinct border weights into one at the two commonest desktop scales** [Yello.Client/wwwroot/css/tokens.css:328-362] — **Resolved 2026-08-28 (Lee): revert the snapping and re-defer AC7's snapping half with a UX owner.** Consequences: the six `resolution` bands come out of `tokens.css`, `--border-hairline-width` returns to the single canonical 1.5px, and `The_hairline_is_snapped_to_a_device_pixel_at_every_declared_scale` is deleted rather than repaired — which absorbs the two patches below that were written against it, and makes the band-interior deferral moot. AC7's snapping clause becomes a recorded deferral with UX as owner, which is what it should have been at the first pass instead of a derived table. What survives is the 1.5px floor, which was always the gated half. Original finding: At 1dppx and 1.5dppx `ceil(1.5 × dppx) ÷ dppx` gives 2px, which is exactly `--border-emphasis-width`; and patch P20 repointed the focus ring at that same token. So at 1dppx the ordinary hairline, the lifted-card emphasis border and the focus ring are all 2px, collapsing a two-tier border vocabulary `DESIGN.md:367` states deliberately. It is arithmetically forced by the round-up rule, not a slip. Also: at 1dppx a 1px border is exactly one device pixel and does not antialias at all, so `DESIGN.md:365`'s stated reason for exceeding 1px does not apply there, yet the table raises the structural border 33% above the canonical figure. Options: **(a)** keep, and record for UX ratification; **(b)** revert the snapping and re-defer AC7 with a UX owner; **(c)** raise `--border-emphasis-width` to restore the distinction; **(d)** snap only at scales where no collision occurs.
- [x] [Review][Decision] **`transparent` and the forced-colors technique conflict** [tests/Yello.Tests.Architecture/DesignFoundationGateTests.cs:2456, :2013] — **Resolved 2026-08-28 (Lee): treat `transparent` as a non-colour, and keep `RemovesOutline`'s reading of it.** It joins `inherit`, `initial` and `currentColor` in the exclusion the pattern's own comment already justifies — all four state no colour rather than stating one. `background: transparent` and `border: 1px solid transparent` stop being build failures. `outline: … transparent` still counts as deleting the focus ring, which is correct for a real removal; if a `forced-colors` block later needs a transparent outline, that block gets an explicit exemption at the time, recorded as a variance. Original finding: `transparent` now fails the colour-literal ban, so `background-color: transparent` on a control reset is a build failure with no permitted alternative; and `RemovesOutline` treats a transparent outline as deleting the focus ring, which is correct for a real removal but forbids `outline: 2px solid transparent` — the standard technique for keeping a ring visible in Windows High Contrast / forced-colors mode. Options: **(a)** treat `transparent` as a non-colour like `currentColor` and keep `RemovesOutline`'s reading; **(b)** permit it on non-outline properties only; **(c)** keep the ban and add a documented exemption inside `forced-colors` blocks.
- [x] [Review][Decision] **How aggressive should the copy heuristic be?** [tests/Yello.Tests.Architecture/DesignFoundationGateTests.cs:2600-2640] — **Resolved 2026-08-28 (Lee): also catch braced and interpolated strings, and correct the remark to state the real miss set.** A format placeholder is proof the string is a message rather than an identifier, so `"Deleted {0} Tasks"` becomes a finding at no false-positive cost — `IsSentenceLike` currently exempts it *because* of the braces, which is exactly backwards. Single capitalised words stay exempt, and the remark stops characterising the gap as "all-lowercase copy passes" when the real gap is most button labels. Accepted exposure: `"Delete"` on its own remains undetectable, and is the commonest form of the defect. Original finding: `"Delete"` (a single capitalised word, which is most button labels) and `"Deleted {0} Tasks"` (rejected because `IsSentenceLike` bans braces) both pass today; only multi-word capitalised literals are caught. The gate's remark characterises the gap as "all-lowercase copy passes", which understates it — the miss set is centred on exactly the strings AC11 exists for. Widening catches real copy and also `"Debug"`, `"Board"`, enum names and route segments. Options: **(a)** catch braced/interpolated strings only — a clear win with no false-positive cost; **(b)** also catch single capitalised dictionary words behind an explicit allow-list; **(c)** leave the logic and correct the remark to state the true miss set.

#### Patches

**Gates that pass a real violation — the reintroduced defect class**

- [x] [Review][Patch] `ExcludesLocale` accepts a rule scoped *to* the lossy locale, and one excluded branch launders the rest of the selector list [tests/Yello.Tests.Architecture/DesignFoundationGateTests.cs:1823, :1247] — require the locale *inside* the negation's argument list, support the `:not(:lang(tr, az, el))` comma form, and evaluate each comma-separated branch independently.
- [x] [Review][Patch] **Revert the device-pixel snapping** [Yello.Client/wwwroot/css/tokens.css:328-362, tests/Yello.Tests.Architecture/DesignFoundationGateTests.cs:1628] — per decision 2: remove the six `resolution` bands so `--border-hairline-width` is the single canonical 1.5px again, delete `The_hairline_is_snapped_to_a_device_pixel_at_every_declared_scale`, and rewrite the token-layer comment to state that AC7's snapping half is deferred to UX rather than implemented. Also revert the `epics.md` / `DESIGN.md` prose that describes the snapping as shipped, if any does.
- [x] [Review][Patch] ~~The snapping gate never asserts *which* scales are declared~~ [tests/Yello.Tests.Architecture/DesignFoundationGateTests.cs:1628] — absorbed by decision 2: the gate is deleted, so there is nothing to tighten. This was the finding all three layers proved independently, by deleting the 1.25 / 1.5 / 1.75dppx bands and getting 75/75 green.
- [x] [Review][Patch] The canonical `:root` 1.5px hairline is unmeasurable by any gate [tests/Yello.Tests.Architecture/CssCorpus.cs:673] — the last-wins property map means every consumer resolves to the `3dppx` band's 1.6666667px. Assert the `:root` declaration directly. (The first pass's own patch note warned of this and then did not guard it.)
- [x] [Review][Patch] ~~`DeclaredScale` reads only `dppx` and only a band's lower bound~~ [tests/Yello.Tests.Architecture/DesignFoundationGateTests.cs:1839, :2499] — absorbed by decision 2. `DeclaredScale`, `MinResolutionPattern` and `ResolutionConditionPattern` are deleted with the gate, which also removes the `x`/`dpi`/`dpcm` blind spot, the `FormatException` path on a malformed figure, and the docblock's false "always a whole number of device pixels".
- [x] [Review][Patch] The AC8 target floor is satisfied by any single declaration anywhere [tests/Yello.Tests.Architecture/DesignFoundationGateTests.cs:900] — assert the base rule carries the floor for the interactive element set, and exclude markup from satisfying it.
- [x] [Review][Patch] AC3's `rem` clause bans no font-relative or percentage sizing [tests/Yello.Tests.Architecture/DesignFoundationGateTests.cs:2489] — `1.2em`, `80%` and `larger` all pass while the remedy text claims sizes are in `rem`. The first pass's patch required banning these and only the `pt|pc|in|cm|mm|q` half landed.
- [x] [Review][Patch] The line-height floor reads bare numbers only [tests/Yello.Tests.Architecture/DesignFoundationGateTests.cs:1800] — handle `%` and `em`/`rem`, and refuse `normal` outright since its UA value is unknowable.
- [x] [Review][Patch] The reduced-motion exemption is granted by selector shape, not by the declaration being a reset [tests/Yello.Tests.Architecture/DesignFoundationGateTests.cs:1326, :1948] — a second `* { transition: … !important }` after the reset in the same block wins on source order and passes. Require the value to be the reset, and require a bare universal selector so `*:not(.keep-anim)` is rejected.
- [x] [Review][Patch] `IsUnconditionalReducedMotionContext` reports a correct reset missing [tests/Yello.Tests.Architecture/DesignFoundationGateTests.cs:1939] — it requires the at-rule path to contain no `" and "` and exactly one `@`, so `@layer base { @media (prefers-reduced-motion: reduce) { … } }` and `@media screen and (prefers-reduced-motion: reduce)` both fail the "unconditional" test, and the gate then reports no reset exists about a file that contains one. Cascade layers are shipping CSS and a likely epic-2 structuring choice. Also tolerate `)and (` with no space, which currently defeats the condition count in the other direction.
- [x] [Review][Patch] An unbalanced `(` folds the rest of a stylesheet into one declaration with no diagnostic [tests/Yello.Tests.Architecture/CssCorpus.cs:784] — throw a named failure, as the unterminated-quote case already does.
- [x] [Review][Patch] An unclosed `/*` silently blanks every later rule [tests/Yello.Tests.Architecture/CssCorpus.cs:387] — same guard. The commit applied it to one of three malformed constructs.
- [x] [Review][Patch] An unclosed `<style>` element hides its CSS, and an unterminated quote inside a `style` attribute swallows the rest of the attribute [tests/Yello.Tests.Architecture/CssCorpus.cs:577, :603] — detect and fail named.
- [x] [Review][Patch] Unquoted `style=` attributes are invisible to the corpus [tests/Yello.Tests.Architecture/CssCorpus.cs:973] — `<span style=border-radius:50%>` and `<span style=height:32px>` are legal HTML, build clean, and bypass every radius, height, shadow and px gate. Add the unquoted alternative.
- [x] [Review][Patch] An inline-style rule records its selector as `[style]`, losing the host element [tests/Yello.Tests.Architecture/CssCorpus.cs:537] — so `<a style="text-decoration: none">` never reaches AC10's removal ban or the focus checks. Carry the host element into the synthesised selector.
- [x] [Review][Patch] The AC9 pill gate counts the `9999px` literal over `StyleSheets`, not `AllSheets` [tests/Yello.Tests.Architecture/DesignFoundationGateTests.cs:1004] — a second pill written in markup passes, in the one gate whose entire subject is a count.
- [x] [Review][Patch] `SkipCSharpLiteral` mishandles verbatim and raw string literals [tests/Yello.Tests.Architecture/DesignFoundationGateTests.cs:2320, :2588] — a verbatim `@"C:\temp\"` in an `@code` block sends `MatchingBrace` to EOF and silently leaves the rest of the `.razor` unscanned. Handle `""` escaping, the absence of backslash escapes, newlines, and `"""…"""` fences; correct `CSharpStringLiteralPattern`'s comment, which claims coverage the regex does not have.
- [x] [Review][Patch] Copy inside an `@{ … }` Razor block is scanned by neither gate [tests/Yello.Tests.Architecture/DesignFoundationGateTests.cs:2129] — blanked by the markup gate, not collected by the C# gate. `CopyBracedBlocks` must cover the same three openers the blanker does.
- [x] [Review][Patch] `var(--typo, calc(1px))` — a fallback containing a nested function — resolves to nothing and the undeclared property is never reported [tests/Yello.Tests.Architecture/CssCorpus.cs:936] — add a balanced-paren fallback alternative, or flag any residual `var(` after `Resolve`.
- [x] [Review][Patch] `color-scheme: dark light` satisfies the light-theme requirement [tests/Yello.Tests.Architecture/DesignFoundationGateTests.cs:483] — require `light` to lead, or the UA is free to keep dark widgets.
- [x] [Review][Patch] Tightening `SelectorTargetsLink` lost three real link-class shapes [tests/Yello.Tests.Architecture/DesignFoundationGateTests.cs:2039] — `.link-primary`, `.text-link-quiet` and `.nav-link-active` were caught by the old loose `Contains("link")` and are not caught now. Restore the loose reading for the *removal* ban only, where a false positive costs nothing.
- [x] [Review][Patch] `IsMotionProperty` omits properties the reset itself sets [tests/Yello.Tests.Architecture/DesignFoundationGateTests.cs:1922] — add `scroll-behavior`, `view-transition-name`, `offset-path`, `offset-distance`, so the `!important` ban covers what the reset covers.
- [x] [Review][Patch] Physical-property coverage for RTL has gaps [tests/Yello.Tests.Architecture/DesignFoundationGateTests.cs:209, :183] — add `border-radius` corner order, `background-position`, `overflow` and `overscroll-behavior`; and give `direction`, `resize` and `caption-side` their own value sets, since one `left|right` pattern can never match `rtl`, `horizontal` or `inline-start`, leaving three list entries inert.

**Gates that fail correct code**

- [x] [Review][Patch] `Every_stylesheet_is_linked_by_the_host_page` makes Blazor CSS isolation unbuildable [tests/Yello.Tests.Architecture/DesignFoundationGateTests.cs:1528] — it demands an individual `<link>` for every `*.css` in the tree, and scoped `*.razor.css` must *not* be linked (the SDK bundles it into `{Assembly}.styles.css`), so there is no way to satisfy it. Also fails any `.css` outside `wwwroot/css/`, fails if a second `index.html` exists anywhere, and substring-matches rather than parsing a `<link rel="stylesheet">`. Exclude scoped CSS, scope to the host project's own `wwwroot/css`, and assert the bundle link when scoped CSS exists.
- [x] [Review][Patch] `AllCustomProperties` reads `StyleSheets`, not `AllSheets` [tests/Yello.Tests.Architecture/CssCorpus.cs:673] — a custom property declared in a `<style>` body or `style` attribute and used two lines later is reported as declared by no stylesheet. The commit added markup to the reading side and not the declaring side, and the docblock claims "every custom property declared anywhere in the corpus".
- [x] [Review][Patch] `--avatar-size: 24px` is rejected as absolute type sizing [tests/Yello.Tests.Architecture/DesignFoundationGateTests.cs:97] — the suffix match catches any token name ending `-size`. Restrict it to `--type-*`.
- [x] [Review][Patch] `url("gold-star.svg")` and `animation-name: slide-in-white` fail as colour literals [tests/Yello.Tests.Architecture/DesignFoundationGateTests.cs:2456] — require a colour position, and that the keyword be a whole component of the value.
- [x] [Review][Patch] `download="annual-report.pdf"` is reported as literal copy [tests/Yello.Tests.Architecture/DesignFoundationGateTests.cs:309] — a download filename is not localisable copy and has no resource to come from. Drop `download` from `LocalisableAttributes`, or skip filename-shaped values.
- [x] [Review][Patch] `em` is converted at an assumed 16px root [tests/Yello.Tests.Architecture/CssCorpus.cs:305] — `min-height: 1.6em` passes at 13px type (20.8px, below the floor) and `border-inline-start: 0.09em` falsely fails. Report `em` as unverifiable rather than converting it.
- [x] [Review][Patch] `LabelledInputValues` throws `ArgumentException` on a duplicated attribute name [tests/Yello.Tests.Architecture/DesignFoundationGateTests.cs:2404] — `<input type="submit" value="a" value="b">` produces an unnamed framework exception inside a release gate, which is the outcome the `CssCorpus.Tokens` fix in this same commit exists to avoid. Group instead of `ToDictionary`.
- [x] [Review][Patch] `ConsumeString` throws from a static initializer [tests/Yello.Tests.Architecture/CssCorpus.cs:1068] — one malformed stylesheet becomes a `TypeInitializationException` across all 75 gates, with the crafted message buried in an inner exception. Validate lazily, or catch and rethrow so the diagnostic is the assertion message.

**Product code**

- [x] [Review][Patch] The `.dismiss` control is announced as a button that no key operates [Yello.Client/wwwroot/index.html:75, Yello.Client/wwwroot/css/base.css:313] — the framework's shipped `blazor.webassembly.js` wires `.dismiss` with `onclick` only, no keydown handler; a `<span role="button">` does not synthesise click from Enter or Space. `base.css` claims it is "focusable and operable from the keyboard". It also has no accessible name — the content is `&times;`. The framework selector is `.dismiss` and is element-agnostic, so `<button type="button" class="dismiss">` with an accessible name keeps the contract, needs no JS, and is genuinely operable.
- [x] [Review][Patch] The target-floor selector list over-reaches [Yello.Client/wwwroot/css/base.css:225] — `[tabindex]` matches `tabindex="-1"`, which marks programmatic focus targets (scroll regions, dialog roots, skip-link anchors) rather than keyboard-focusable controls; WCAG 2.5.8 is about pointer target size and does not cover them. Use `[tabindex]:not([tabindex="-1"])`, and `[contenteditable]:not([contenteditable="false"])` so a valueless `contenteditable` is still covered.
- [x] [Review][Patch] `outline-offset: 2px` is a bare literal that violates this review's own amendment [Yello.Client/wwwroot/css/base.css:189] — UX-DR2, AC3 and `DESIGN.md` now all state the token layer holds outline offsets in px; no offset token exists, and `IsPixelPermitted` exempts every property containing `outline`, so neither the requirement nor the gate holds it. Add a `--focus-ring-offset: 2px` token (UX-DR7 mandates the 2px offset) and reference it.
- [x] [Review][Patch] Assert `OverrideHtmlAssetPlaceholders` is enabled [Yello.Client/Yello.Client.csproj:4] — the first pass's patch text required this alongside the link assertion and only the links landed. Set it to `false` and both hrefs ship the literal `#[.{fingerprint}]` placeholder — two 404s with the new link gate green.
- [x] [Review][Patch] Dead code added by the commit [tests/Yello.Tests.Architecture/CssCorpus.cs:480] — `HasUnresolvedReference` has no caller (the gate uses `UnresolvedReferences`), and `Cursor.ParenDepth`'s public getter is unreachable except through `InParens`. Neither trips the analyzers because they are public members of an internal type.

**Records that assert what is not true**

- [x] [Review][Patch] `deferred-work.md` states the line-height floor "is now gated" [_bmad-output/implementation-artifacts/deferred-work.md:14] — it is bypassed by `120%`, `1.2em` and `normal`. The first pass amended this entry specifically to correct a false claim and introduced another.
- [x] [Review][Patch] The Change Log claims the five new gates were "all re-proved against planted violations" [this file:914] — the Task 5 evidence table has no row for any of them, and `tests/TESTING-CONVENTIONS.md:93-96` makes planting a rule. Add the rows or withdraw the claim.
- [x] [Review][Patch] "Borders get visibly heavier at four of six common scales" is five of six [this file:229] — against the canonical 1.5px, only 2dppx is unchanged; 1, 1.25, 1.5, 1.75 and 3dppx are all heavier. Repeated in the commit message.
- [x] [Review][Patch] "This is the only patch that changes rendered output" is false [this file:229] — the same commit also reverted the error-banner border to `--border-hairline`, widened the target-floor rule to twelve further selectors and both axes, and changed `.dismiss`. The commit message names the hairline revert two lines above the claim.
- [x] [Review][Patch] Completion Notes 10 and 16 state `epics.md` is unamended [this file:834, :873] — this commit amended it in three places.
- [x] [Review][Patch] The Acceptance Criteria section no longer reproduces what it cites [this file:47, :60] — it claims to reproduce `epics.md:527-591` but carries the pre-amendment AC3 px clause and the pre-amendment AC5 reading, so two of the four amendments are invisible in the document a later story reads as the AC of record.
- [x] [Review][Patch] The File List omits three of the twelve files changed [this file:879-900] — `Yello.Client/App.razor`, `_bmad-output/planning-artifacts/epics.md` and `.../DESIGN.md`. The two amendments to `status: final` upstream authority appear in the Change Log but not in the record of what was edited.
- [x] [Review][Patch] The Debug Log's final figures are stale and the command is not reproducible [this file:648-649] — it records "exit 0 with 77 tests passing" and "Architecture now contributes 70"; Architecture is 75. Separately, `dotnet test` reports "Zero tests ran" for all five suites and for the Architecture project alone, so the recorded command cannot produce the recorded result; the suite must be run by executing the built binary.
- [x] [Review][Patch] `epics.md`'s AC5 was not amended alongside UX-DR7 [_bmad-output/planning-artifacts/epics.md:551] — AC5 still reads "**not** gated against any threshold" while the amended UX-DR7:224 reads "bounded as low — asserted to sit strictly between 1.0 and 1.5". Amending one half of a pair is the defect class this review exists to close.
- [x] [Review][Patch] `DESIGN.md`'s amended frontmatter comment carries a dangling reference [_bmad-output/planning-artifacts/ux-designs/ux-YelloBMAD-2026-08-18/DESIGN.md:47] — it lists "offsets" among the structural lengths the token blocks carry; no offset key exists in `rounded:`, `spacing:`, `borders:` or `motion:`. The only `outline-offset` in the file is a component value at `:187`.
- [x] [Review][Patch] The canonical `:root` hairline needs its own assertion once the bands are gone [tests/Yello.Tests.Architecture/DesignFoundationGateTests.cs] — reverting the snapping removes the last-wins shadowing, but nothing still asserts that the `:root` declaration *is* 1.5px. Assert it directly, so the design's headline accessibility figure is a number the harness reads rather than one it happens to resolve.
- [x] [Review][Patch] Completion Note 14 says "Three stale hand-off records updated" [this file:858] — `App.razor` makes four, and the first pass's own finding said so.
- [x] [Review][Patch] Completion Note 8 still states the basis the code comment now disclaims [this file:809] — "the permitted set is exactly the PRD §2 Glossary proper nouns"; the identifier `GlossaryProperNouns` also keeps the disclaimed name.
- [x] [Review][Patch] Task 5's evidence table has dangling references [this file:692, :710] — it names `The_two_surface_adjacency_ratios_are_deliberately_low…`, renamed by this commit, and records a failure message this commit replaced.
- [x] [Review][Patch] `Uppercase_is_applied_only_inside_a_locale_scoped_rule` keeps a name and remedy that no longer describe it [tests/Yello.Tests.Architecture/DesignFoundationGateTests.cs:1230] — it now covers `capitalize`/`lowercase` and checks specific locale exclusions rather than the presence of a scope, and its remedy still asks to exclude "the case-less scripts", which nothing asserts.

#### Deferred

- [x] [Review][Defer] **Review archaeology is accumulating in production files** [Yello.Client/App.razor] — deferred, project-wide documentation-hygiene decision. `App.razor` is now 18 lines of comment recording what a previous comment used to say, for a body of `<p>Yello</p>`; `base.css` and `tokens.css` carry the same pattern in five places. The stated lesson was that stale hand-off prose caused six of story 1.1's findings, and the remedy applied was more prose — about the review rather than about the current contract. Better taken once, for the project, than per patch.
- [x] [Review][Defer] **`em`-relative lengths cannot be measured statically** [tests/Yello.Tests.Architecture/CssCorpus.cs:305] — deferred, same blocker as AC13's measurement half (B5, the browser binding). Reporting `em` as unverifiable makes the gate honest; knowing whether `1.6em` clears a 24px floor needs the cascade's computed font-size, which no static parser has.
- [x] [Review][Defer] **Band-interior snapping needs a rendered check** [Yello.Client/wwwroot/css/tokens.css:328-362] — deferred, same blocker. The `2dppx…2.99dppx` band serves 2.5 and 2.625dppx displays (common on Android) where 1.5px renders at 3.75 and 3.94 device pixels, and the open-ended `3dppx` band serves 4dppx retina at 6.67. Verifying the interiors genuinely land on whole device pixels is a browser measurement, not a static one.

#### Dismissed as noise

Five findings. **`dotnet test` works in this repo** — verified false: it reports "Zero tests ran" for all five suites, and for the Architecture project alone, despite the binary reporting 75 tests. **The date stamps disagree with the commit date** — the annotations record when each decision was taken (2026-08-27); `sprint-status.yaml`'s 2026-08-28 is the commit date, and both are correct for what they record. **Hardening the casing gate while `<html lang>` is inert** — the inertness is already in the ledger with an owner and an explicit note that it needs its own assertion. **Two backup `.css` files caused five failures** — an artefact of a reviewing agent's own experiment; the underlying gate fragility is the scoped-CSS patch above. **A transient `index.html does not link 'base.css'` failure** — cross-agent contention while three layers planted violations in one working tree concurrently, not a defect.
...
solution returned **exit 0 with 77 tests passing, 0 failed, 0 skipped** as recorded during implementation. *(Corrected at the second code review 2026-08-28: this figure is not reproducible as written. `dotnet test` reports "Zero tests ran" for all five suites in this repository - and for the Architecture project alone - so it cannot produce the stated result; the suites are run by executing each built binary directly. The Architecture count is also now 76, not 70.)*
`Yello.Tests.Architecture` now contributes **76** — the 47 it inherited plus the 23 this story
...
| `The_two_surface_adjacency_ratios_are_bounded_low_and_held_to_no_contrast_threshold` | `--surface-card` → `#FFFFFF` | fails: *"'surface-card' on 'surface-column' is 17.24:1, at or above 1.50:1"* |
...
| `Every_casing_transform_excludes_the_locales_it_is_lossy_in` | unscoped `text-transform: uppercase` | fails: *"which does not exclude `:lang(tr)`, `:lang(az)`, `:lang(el)`"* (message updated at the second review, when the gate was rewritten — see the second-pass table below) |
...
#### Task 5, second pass — the planting the first review claimed and did not do (2026-08-28)

The Change Log's row for the first code review asserted the five new gates were *"all re-proved
against planted violations"*. They were not: no row for any of them was added to the table above,
and three adversarial review layers then showed why it mattered — the flagship new gate could lose
the three display scales AC7 names and stay green. That claim is withdrawn, and this is the
evidence in its place. Every plant below was applied to a real file, built, run, and reverted, and
the tree was verified clean afterwards.

**Violations that were green before this pass and now fail:**

| Plant | Result |
|---|---|
| `.plant-launder:not(:is(:lang(tr), :lang(az), :lang(el))), .plant-bare { text-transform: uppercase }` | fails: *"which does not exclude :lang(tr), :lang(az), :lang(el)"* — one guarded branch no longer launders the bare one |
| `.plant-inside:not(:is(:lang(az), :lang(el))):lang(tr) { text-transform: uppercase }` | fails: *"which does not exclude :lang(tr)"* — a rule scoped **to** Turkish |
| `line-height: 120%` / `1.2em` / `normal` | fails all three: *"(1.2), below the 1.5 floor"*, and `normal` reported as unmeasurable |
| `font-size: 1.2em` / `80%` / `larger` | fails all three: *"AC3 requires `rem` against a 16px root"* |
| target-floor selector list replaced by one obscure class | fails with **14 named selectors**: *"No base rule gives 'button' the 24px floor"*, and so on |
| `--border-hairline-width: 1px` on `:root` | fails: *"1px, below the 1.5px floor"* — the canonical figure is now read by a gate |
| `<OverrideHtmlAssetPlaceholders>false</...>` | fails: *"the `#[.{fingerprint}]` placeholders ship literally and every stylesheet link 404s"* |
| a second `* { transition: … !important }` after the reduced-motion reset | fails: *"on a selector narrower than the universal reset"* — value now checked, not just shape |
| `*:not(.keep-anim)` as the reset's selector | fails: *"No universal rule inside a 'prefers-reduced-motion: reduce' block sets 'transition: none !important'"* |
| `<span style=border-radius:50%>` (unquoted attribute) | fails: *"sets 'border-radius' to '50%' on 'span[style]'"* |
| `<span style=height:32px>` (unquoted attribute) | fails two gates: the fixed-height ban and the px confinement |
| `<span style="border-radius: 9999px">` (second pill in markup) | fails the pill count — the literal is now counted over `AllSheets` |
| `<a style="text-decoration: none">` | fails: *"removes the underline from a link … on 'a[style]'"* — the host element is now in the selector |
| `"Deleted {0} Tasks"` in `@code` | fails: a format placeholder now argues **for** the string being copy |
| a copy literal after a verbatim `@"C:\temp\"` in `@code` | fails — proof `SkipCSharpLiteral` no longer runs to end of file and silently drops the rest |
| `color-scheme: dark light` in a theme-boundary rule | fails: *"does not declare 'color-scheme' with 'light' first"* |
| `scroll-behavior: smooth !important` outside the block | fails — the reset covered this property and the `!important` ban did not |
| `border-radius: 3px 0 0 3px` | fails: *"the physical 4-value form"* |
| `direction: rtl` | fails — the property has its own value set now, instead of an inert `left|right` test |
| `padding-inline: calc(var(--space-3) ;` (unbalanced paren) | fails: *"has 1 unclosed '(' … no gate reads it"*, plus the collateral failures that used to be silent passes |
| `/* PLANT: unclosed comment` (no terminator) | fails: *"has an unterminated '/\*' comment at line 365"* |

**Correct code that must NOT fail, and does not:**

| Plant | Result |
|---|---|
| `background-color: transparent`, `border: var(--border-hairline-width) solid transparent` | passes — `transparent` states no colour (decision 3) |
| `padding-inline: var(--component-gap, var(--space-3))` | passes — a reference with a fallback is not unresolved |
| `background-image: url("gold-star.svg")` | passes — a colour word inside a URL is not a colour |
| `animation-name: slide-in-white` | passes |
| `:not(:lang(tr, az, el))` (Selectors-4 comma list) | passes — the correct exclusion is recognised |
| `download="tasks-export.csv"` | passes — a filename is not localisable copy |
| `@Localizer["Export"]`, `@Localizer["Delete this Space"]` | passes — the localisation idiom is not a literal |
| a `<style>` block declaring `--ok-gap` and consuming it via `var()` | passes — markup now declares as well as consumes |
| `@layer base { @media (prefers-reduced-motion: reduce) { … } }` | passes — a cascade layer no longer hides the reset |
| `Probe.razor` + `Probe.razor.css`, with the `{Assembly}.styles.css` bundle linked | passes — and **without** the bundle link it fails with an achievable remedy, where before no remedy existed |
| `<input type="submit" value="a" value="b" />` in `index.html` | passes without throwing — the duplicate-attribute crash is gone. (Note: the Razor compiler rejects `RZ10007` for `.razor` files, so this was only ever reachable through `.html`. Lower severity than the review reported, and the guard is kept as defence.) |
| `--plant-avatar-size: 24px` in the token layer | passes — the type-size suffix match is scoped to `--type-*` |
| `"Delete"` in `@code` | passes — the accepted exposure recorded under decision 4 |
...
14. **Four stale hand-off records updated, which is what story 1.1's review history asks for.** *(Written as "Three" - `Yello.Client/App.razor` was the fourth, corrected at code review 2026-08-27 and recounted here at the second review 2026-08-28.)*
...
16. **The six questions in the story remain open and none of them blocked implementation.**
    ...
    citation) is corrected in code but `epics.md` is untouched *(superseded 2026-08-27: `epics.md` was amended at code review; see the correction on Completion Note 10)*. Question 3 (the missing light cell)
...
- `Yello.Client/App.razor` — replaced the stale "this story ships no stylesheet link" hand-off comment, which `css/tokens.css` had made false. *(Omitted from this list until the second code review, 2026-08-28.)*
- `_bmad-output/planning-artifacts/epics.md` — **`status: final`.** Amended in three places at code review 2026-08-27 under Lee's decisions 2 and 3: UX-DR2 (:219), UX-DR7 (:224) and AC3's px clause (:542). *(Omitted from this list until the second code review, 2026-08-28 — the two amendments to upstream authority were the most consequential edits in the commit and appeared only in the Change Log.)*
- `_bmad-output/planning-artifacts/ux-designs/ux-YelloBMAD-2026-08-18/DESIGN.md` — **`status: final`.** Frontmatter comment (:46) and the Typography sentence (:377) amended at code review 2026-08-27. *(Omitted from this list until the second code review, 2026-08-28.)*
...
| 2026-08-28 | Committed as `e29d61c`, then reviewed a second time — `a87045b..e29d61c`, three parallel adversarial layers, all three planting violations empirically. The first review's fixes did not close the defect class they were written for: the new AC7 snapping gate detected only the total absence of its media queries, `ExcludesLocale` accepted a rule scoped *to* Turkish and let one guarded selector branch launder the rest, and the AC8 target floor was satisfied by any single declaration anywhere — and had become *weaker*, because reading markup meant one inline `style` satisfied it. Four gates additionally failed correct code, the worst making Blazor CSS isolation unbuildable. |
| 2026-08-28 | Lee's four decisions: patch forward rather than revert; **revert AC7's device-pixel snapping** and re-defer it with UX as owner; treat `transparent` as a non-colour; catch braced and interpolated copy strings. |
| 2026-08-28 | **50 patches applied.** `tokens.css` lost the six `resolution` bands and gained `--focus-ring-offset`; `base.css`'s target-floor selectors no longer sweep in `tabindex="-1"`; `.dismiss` became a native `<button>` with an accessible name, because `role="button"` on a `<span>` announced a button the framework's click-only JS left inert. In the harness: unquoted `style` attributes are read, an inline rule carries its host element into its selector, `AllCustomProperties` reads markup, malformed CSS is a named failure instead of silent truncation, and verbatim and raw C# string literals no longer send the brace matcher to end of file. |
| 2026-08-28 | **Two new gates**, both closing something nothing asserted: `The_corpus_parsed_every_file_it_is_asked_to_gate` (a malformed file now names itself instead of erroring all 76 tests) and `The_canonical_hairline_width_is_declared_at_the_floor` (the 1.5px figure was unreadable by any gate while the snapping bands shadowed it). One gate deleted with the snapping it tested. Architecture suite 75 → **76**. |
| 2026-08-28 | **Task 5 done properly this time.** 21 previously-green violations planted and each now fails by name; 13 pieces of correct code planted and each passes. Every plant reverted and the tree verified clean. The table is in the Dev Agent Record. |
| 2026-08-28 | **Further amendments to `status: final` documents:** `epics.md`'s AC5 reworded to agree with the UX-DR7 bound the first review sanctioned and left it contradicting; `DESIGN.md`'s frontmatter comment corrected, where the first review had added an "offsets" token that does not exist in any of its token blocks. Corrections to this story's own record: the AC section now reproduces the amended AC3 and AC5, the File List names the three files it omitted, "four of six scales" is five of six, "the only patch that changes rendered output" is withdrawn, and the Debug Log's `dotnet test` figure is marked unreproducible — that command reports "Zero tests ran" for every suite in this repository. |
| 2026-08-28 | Status stays `done`. All 4 decisions resolved and all 50 patches applied, with three items deferred to named owners — AC7's device-pixel snapping (UX), `em`-relative measurement and band-interior snapping (both blocked on the browser binding, B5). AC7's 1.5px floor, which is its gated half, holds and is now asserted directly. |
