...
scope: 'Yello v1 — the whole system. FR-1…FR-43, NFR-1…NFR-9. Reconciled against prd.md as revised 2026-08-20.'
status: final
created: '2026-08-17'
updated: '2026-08-20'
binds:
...
- **Prevents:** each slice inventing its own refusal code and disclosing existence across Spaces; and a route shape that turns AD-20's refusal record into a timing oracle.
- **Rule:** A resource in a Space the caller holds no Membership in returns **404**, identically to a resource that does not exist. A resource in a Space the caller does belong to, refused for Role, returns **403**. No handler converts one into the other. Error bodies carry no Space name, resource title, or existence hint.
  - **Every Space-scoped route carries `{spaceId}` in the path** — the Consistency Convention is load-bearing for isolation, not just for naming, and an architecture test (AD-21) fails the build on a Space-scoped endpoint that resolves a `Task`, `Project`, `Label` or `StatusDefinition` without it. This is what keeps AD-20's asymmetry harmless: because the caller always supplies the Space, the boundary refusal that writes a record fires on exactly the condition the caller already knows — whether they hold a Membership there. A bare-id route (`/tasks/{taskId}`, a shareable deep link) would let the server resolve the owning Space itself, and then a record-writing boundary refusal and a recordless ordinary not-found become separable **by duration**, disclosing that the id exists somewhere in Yello. AD-23 already treats duration as part of its contract; that discipline is met here structurally instead, by removing the case rather than by padding it.
  - The isolation suite (AD-21) carries a **timing** case, not only a disclosure case: a boundary 404 and an in-Space not-found are compared for duration as well as for body. SM-1 as originally scoped measures what a response *says*, not how long it takes to say it.
...
- **Binds:** FR-29, FR-35, and AD-29's read path
- **Prevents:** integer positions and bulk renumbering, which make concurrent moves discard one another; and a key whose ordering the database does not actually reproduce.
- **Rule:** `Task` carries a lexicographically sortable position key unique within `(ProjectId, StatusId)`, generated by fractional indexing with jitter. A move writes only the moved Task's key — never a renumber of siblings. The key is readable over the API and not writable (FR-35).
  - **The key column is declared `COLLATE Latin1_General_100_BIN2`, and the migration is what enforces it.** "Lexicographically sortable" is a property of the *algorithm*; whether the database reproduces that order is a property of the *collation*, and the two do not agree by default. Azure SQL Database's default is `SQL_Latin1_General_CP1_CI_AS` — **case-insensitive** — under which a mixed-case fractional-index alphabet (base62 is the usual choice) makes `a0` and `A0` compare **equal**: `ORDER BY` becomes non-deterministic between them and the uniqueness constraint rejects the second as a duplicate. A binary collation compares by code point, which is exactly what the algorithm assumes. `ALTER DATABASE … COLLATE` is not supported on Azure SQL Database, so this is set at column definition and cannot be retrofitted server-wide later.
  - Same collation on the `(ProjectId, StatusId, PositionKey)` index that AD-29 seeks against — a keyset seek of `PositionKey > @last` is wrong in precisely the same way if the comparison is case-insensitive.
...
- **Rule:** An `OwnershipOffer` carries `SpaceId`, the named recipient `MembershipId`, `ExpiresAt`, and a `State` of `Pending`, `Accepted`, `Declined`, `Revoked` or `Lapsed`. A filtered unique index on `OwnershipOffer(SpaceId) WHERE State = Pending` guarantees at most one pending offer per Space — the same schema-level guarantee AD-5 gives Owner uniqueness, for the same reason. **Accepting or declining is authorised by row identity — is the caller the named Membership — never by Role.** FR-8 permits any Role to be named, so this is the only capability in Yello decided outside the FR-16 Role matrix. Every transition is guarded in the slice by `WHERE State = Pending` with a rowcount check, not by the endpoint's `Idempotency-Key` alone (AD-18 still applies on top), because FR-42 admits no route — the API included — by which ownership arrives unrequested. Acceptance performs the Role change as **two `UPDATE` statements inside one transaction, in this order**: demote the current Owner to `Admin`, *then* promote the recipient to `Owner`. The order is load-bearing, not a style choice. SQL Server has no deferred constraint enforcement — uniqueness is checked per row as the index is maintained, and `SET CONSTRAINTS … DEFERRED` does not exist — so promote-before-demote transiently writes a second `Owner` row for the Space and fails on AD-5's filtered unique index. Demote-first is safe because that index is **filtered**: demoting removes the row from it entirely, leaving zero matching rows, and zero never violates uniqueness. This is also why the usual swap-via-temporary-value dance is unnecessary here. Performing the change through tracked-entity `SaveChanges` is **forbidden**: EF Core picks its own statement order for two tracked rows, so correctness would rest on an ordering it does not guarantee — use two explicit ordered `ExecuteUpdate` calls. FR-42's *never zero or two Owners* is an invariant on **observable** state, delivered by the single transaction; splitting it across two transactions violates it, and an invariant test asserts no Space ever holds zero or two Owner Memberships — the same gating pattern AD-17 uses for Status. A transition refused because the offer is no longer `Pending` — already accepted, declined, revoked, lapsed, or lost to a concurrent offer hitting the filtered index — returns **409** with a stable problem `type`, never 404: the caller holds a Membership in the Space, so AD-3's boundary rule does not apply and inventing a 404 here would be a divergence, not a disclosure. The named recipient reads a pending offer **inside the Space's own context** under AD-2, at whatever Role they hold; this deliberately needs no third Account-scoped surface, so AD-24 stands unamended and an offers inbox spanning Spaces is not the way to surface this.
  - **Exactly one notification per offer (FR-43).** Creating the offer enqueues one email through the existing outbox in the **same transaction** as the `OwnershipOffer` insert, dispatched in-process on enqueue — the AD-10 path already built for FR-39, so no new mechanism and no timer, and AD-10's no-timer rule is untouched. Nothing is emitted on revoke, decline, or lapse. For lapse-by-time that is not a policy choice but a structural consequence: AD-27 computes expiry on read and never writes it, so there is **no event to hang a notification on** — which is the same asymmetry AD-27 already records, seen from the notification side. The email's content constraint is FR-43's, and it is load-bearing here because delivery is irreversible: AD-5 lapses a pending offer when the recipient's Membership ends, but an email already sent cannot be recalled, so anything it carried outlives the Membership that justified carrying it.
...
### AD-29 — Board and List View read paths are keyset-paginated on the position key; nothing is virtualised

- **Binds:** FR-28, FR-29, FR-30, FR-35 — NFR-5, NFR-8, NFR-9
- **Prevents:** the three-way deferral in which the PRD hands Board rendering to the architecture, the architecture never decides it, and the UX spine defers back — leaving the one requirement the PRD flags as unsatisfiable naively owned by nobody. Also prevents the two mechanisms an implementer would otherwise reach for, both of which fail a stated requirement.
- **Rule:** A Board column and a List View page are read by **keyset (seek) pagination on the AD-15 position key**, never by `OFFSET`, and the rendered rows are **appended to the DOM and never recycled**.
  - **Why keyset.** AD-15 already gives every Task a lexicographically sortable key unique within `(ProjectId, StatusId)`. A page is therefore an index seek of bounded cost — `WHERE (ProjectId, StatusId) = … AND PositionKey > @last ORDER BY PositionKey` — whose latency is independent of how deep in the column it sits. `OFFSET` degrades linearly with depth, so at NFR-8's 5,000 Tasks the last page of a column would blow NFR-5's 300 ms p95 read budget on 0.5 vCore with `MAXDOP = 1` (AD-2), while the first page passed. A budget that holds only near the top of a column is not a budget.
  - **Why not DOM virtualisation**, which is the conventional answer and is forbidden here: a virtualiser recycles row elements as the window moves, and a recycled row silently re-points keyboard focus at a different Task — so the next keyboard move operates on the wrong Task. That is a data-corrupting defect reachable only by keyboard, which is to say only by the users NFR-9 exists to protect, and invisible to pointer testing. Appending never recycles, so the defect cannot occur rather than being mitigated.
  - **The count is separate from the page.** A column header states the **true total** for that Status (`COUNT` over the same indexed predicate), never the number loaded. `aria-setsize` carries that total and `aria-posinset` the Task's true ordinal, so assistive technology and the visible chip agree — the UX spine requires this and it is only satisfiable because the count is queried rather than derived from the page.
  - **Consistency with the other view is deliberate.** FR-30 requires the List View to hold the same bounds *"on the same terms as FR-28"*, and the UX spine independently chose paging there for the focus-identity reason above. One mechanism serves both views, and the UX ban on infinite scroll is what makes an explicit page boundary coherent rather than a regression.
  - **But the List View does not sort by the position key**, and keyset pagination is only correct on a sort that is **total**. FR-30 sorts by Status, Assignee, due date or Label — all non-unique, and due date nullable — so a naive keyset seek on any of them silently skips or repeats rows across a page boundary wherever values tie. Every List View keyset is therefore `(sortColumn, TaskId)` with `TaskId` as the mandatory tiebreaker, the seek predicate compares the pair, and the supporting index carries both columns in that order. Nullable sort columns fix `NULL` at one end explicitly rather than relying on the default. A List View sort offered in the interface without a matching composite index is a defect, not a slow query — at 5,000 Tasks it is an NFR-5 failure.
  - `[ASSUMPTION: initial page size 50 per column. Chosen so a three-to-five Status Board's first paint is one bounded seek per column at the specified density, not from measurement. Tunable without changing this AD; the seek shape is the invariant, the page size is not.]`
...
| 4.8 Board and List Views | `Application/Boards/*`, `Application/Lists/*` | AD-13, AD-15, AD-16, AD-29 |
...
| 4.11 Notifications | `Infrastructure/Outbox`, `Infrastructure/Email` | AD-10, AD-23, AD-26 |
