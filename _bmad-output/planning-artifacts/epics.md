...
UX-DR2: Implement the 8-role type scale — `task-title`, `column-head`, `space-name`, `body`, `dialog-title`, `meta`, `role-label`, `presence-count` — sized in **rem against a 16px root** with every line-height ≥ 1.5. Two system stacks (`system-sans`, `system-mono`), no webfont. `px` never sizes or spaces type, in any file; structural lengths that must not scale with text — the spacing scale, the target floor, hairlines, radii, outline offsets and the long-press slop — are stated in `px` in the token layer, and every other file takes its lengths from a token. *(Amended 2026-08-27 at story 1.2's code review: the original wording, "`px` survives only on hairlines, radii and outline offsets", contradicted `DESIGN.md:99-113`, which states the spacing scale, the 24px target floor and the 10px long-press slop in `px` on purpose.)* Metadata is monospace; `presence-count` is deliberately sans, because NFR-9 rests on that string.
...
**And** `px` never sizes or spaces type in any file, and outside the token layer every length comes from a token — the token layer itself states the spacing scale, the target floor, hairlines, radii, outline offsets and the long-press slop in `px`, because those are structural steps that must not scale with text *(amended 2026-08-27 at code review; the original clause, "`px` appears only on hairlines, radii and outline offsets — never on type", contradicted the token values `DESIGN.md:99-113` fixes)*
